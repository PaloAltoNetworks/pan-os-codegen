...
func (c *XmlApiClient) logSend(data url.Values) {
	b := &strings.Builder{}

	if c.Logging["send"] {
		c.logSendData(b, data)
	}
	if c.Logging["curl"] {
		c.logDataAsCurl(b, data)
	}
...
func (c *XmlApiClient) logSendData(b *strings.Builder, data url.Values) {
	if b.Len() > 0 {
		fmt.Fprintf(b, "\n")
	}
...
func (c *XmlApiClient) logDataAsCurl(b *strings.Builder, data url.Values) {
	if b.Len() > 0 {
		fmt.Fprintf(b, "\n")
	}
	...
	// Headers.
	if len(c.Headers) > 0 && c.Logging["personal-data"] {
		for k, v := range c.Headers {
			if v != "" {
...
func (c *XmlApiClient) logReceive(body []byte) {
	b := &strings.Builder{}

	if c.Logging["receive"] {
		c.logReceiveData(b, body)
	}
...
func (c *XmlApiClient) logReceiveData(b *strings.Builder, body []byte) {
	if b.Len() > 0 {
		fmt.Fprintf(b, "\n")
	}
