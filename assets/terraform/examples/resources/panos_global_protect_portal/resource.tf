...
resource "panos_zone" "gp_zone" {
  location = { template = { panos_template.gp_template.name }}
  name     = "gp-zone"
}

# GlobalProtect Portal with Client Config
resource "panos_global_protect_portal" "client_config_example" {
  name     = "gp-portal-client-config"
  template = panos_template.gp_template.name

  client_config = {
    agent_user_override_key = "override-key"
    configs = [
      {
        name                    = "config1"
...
resource "panos_global_protect_portal" "clientless_vpn_example" {
...
resource "panos_global_protect_portal" "portal_config_example" {
...
resource "panos_global_protect_portal" "satellite_config_example" {
