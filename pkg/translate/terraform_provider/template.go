  ...
  {{- if eq .Type "types.Object" }}
	var {{ .Name.LowerCamelCase }}_object types.Object
	{
		...
		if diags_tmp.HasError() {
			errMsg := ""
			for _, elt := range diags_tmp {
				errMsg += fmt.Sprint("\n---\n%s\n%s\n", elt.Summary(), elt.Detail())
			}
			return fmt.Errorf("Failed to unmarshal JSON document into location: %s", errMsg)
		}
	}
  {{- else if eq .Type "types.List" }}
  ...
  {{- if eq .Type "types.Object" }}
	var {{ .Name.LowerCamelCase }}_object *{{ .StructName }}
		...
		if diags.HasError() {
			...
			for _, elt := range diags.Errors() {
				diagErrors += fmt.Sprintf("---\n%s\n%s\n", elt.Summary(), elt.Detail())
			}
			return nil, fmt.Errorf("Failed to marshal location into JSON:\n%s", diagErrors)
		}
        }
  {{- else if eq .Type "types.List" }}
...
{{- else if eq .ResourceType "entry-plural" }}
	...
	var namesObj types.List
	{
		...
		if diags_err.HasError() {
			var errMsg string
			for _, elt := range diags_err.Errors() {
				errMsg += fmt.Sprintf("\n%s\n%s\n", elt.Summary(), elt.Detail())
			}
			return nil, fmt.Errorf("Failed to generate a list of names for the import ID: %s", errMsg)
		}
	}
...
{{- else if or (eq .ResourceType "uuid") }}
	...
	if diags_tmp.HasError() {
		errorSummary := ""
		for _, elt := range diags_tmp.Errors() {
			errorSummary += fmt.Sprintf("%s\n", elt.Summary)
		}
		return nil, fmt.Errorf("Failed to generate import ID: %s", errorSummary)
	}
